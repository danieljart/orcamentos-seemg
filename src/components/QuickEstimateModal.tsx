...
export function QuickEstimateModal({ onClose }: { onClose: (items?: CartItem[]) => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-6xl h-[85vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <Calculator className="text-emerald-600" /> Orçamento Rápido
          </h2>
          <button onClick={() => onClose()} className="text-slate-400 hover:text-slate-600 transition-colors p-1 bg-white rounded-full shadow-sm">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            <div className="p-4 border-b border-slate-100 bg-white">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all text-slate-700"
                  value={searchTerm}
                  onChange={e => {
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {filteredCategories.length > 0 ? (
                filteredCategories.map(cat => {
                  ...
                  return (
                    <div key={cat.item} className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm">
                      <div 
                        className={`flex items-center gap-3 p-3 cursor-pointer hover:bg-slate-50 transition-colors ${isExpanded ? 'bg-slate-50 border-b border-slate-100' : ''}`}
                        onClick={() => toggleCategory(cat.item)}
                      >
                        {isExpanded ? <ChevronDown size={18} className="text-slate-400" /> : <ChevronRight size={18} className="text-slate-400" />}
                        <span className="font-mono text-sm bg-slate-100 text-slate-600 px-2 py-0.5 rounded">{cat.item}</span>
                        <span className="font-bold text-slate-700 text-sm uppercase tracking-wide">{cat.description}</span>
                      </div>
                      {isExpanded && (
                        <div className="p-3 bg-white space-y-2">
                          {cat.children.map(item => {
                            ...
                            return (
                              <div key={item.item} className={`flex justify-between items-center gap-4 p-3 rounded-xl border transition-all ${cartItem ? 'border-emerald-300 bg-emerald-50/20' : 'border-slate-100 bg-slate-50/50 hover:border-slate-300'}`}>
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2 mb-1">
                                    <span className="font-mono text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded">
                                      {item.item}
                                    </span>
                                    <span className="font-bold text-slate-800 text-sm">
                                      {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(item.price)} <span className="text-slate-500 font-normal text-xs">/ {item.unit}</span>
                                    </span>
                                  </div>
                                  <p className="text-sm text-slate-600 leading-snug line-clamp-2" title={item.description}>{item.description}</p>
                                </div>
                                <div className="flex items-center gap-2 bg-white rounded-lg border border-slate-200 p-1 shadow-sm">
                                  {cartItem ? (
                                    <>
                                      <button onClick={() => handleDecrement(item)} className="p-1 hover:bg-slate-100 rounded text-slate-600">
                                        <Minus size={16} />
                                      </button>
                                      <input 
                                        type="text"
                                        ...
                                        className="w-12 text-center text-sm font-bold text-slate-700 focus:outline-none"
                                      />
                                      <button onClick={() => handleIncrement(item)} className="p-1 hover:bg-slate-100 rounded text-slate-600">
                                        <Plus size={16} />
                                      </button>
                                    </>
                                  ) : (
          ...
          <div className="w-96 bg-white flex flex-col shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] z-10">
            <div className="p-4 border-b border-slate-100 flex items-center gap-2 bg-emerald-50">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {cartItems.length > 0 ? (
                cartItems.map(item => (
                  <div key={item.item} className="flex flex-col gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100">
                    <div className="flex justify-between items-start gap-2">
                      <span className="text-xs font-mono text-slate-500">{item.item}</span>
                      <button onClick={() => updateCart(item, '0')} className="text-slate-400 hover:text-red-500">
                        <X size={14} />
                      </button>
                    </div>
                    <p className="text-sm font-medium text-slate-700 line-clamp-2">{item.description}</p>
                    <div className="flex justify-between items-end mt-1">
                      <div className="flex items-center gap-1 bg-white border border-slate-200 rounded p-0.5">
                        <button onClick={() => handleDecrement(item)} className="p-0.5 hover:bg-slate-100 rounded text-slate-600"><Minus size={14}/></button>
                        <span className="text-xs font-bold w-8 text-center">{item.quantity}</span>
                        <button onClick={() => handleIncrement(item)} className="p-0.5 hover:bg-slate-100 rounded text-slate-600"><Plus size={14}/></button>
                      </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 space-y-4">
              ...
              <button 
                ...
                className={`w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md ${cartItems.length > 0 ? 'bg-emerald-600 hover:bg-emerald-700 text-white hover:shadow-lg active:scale-[0.98]' : 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none'}`}
              >
                <Check size={20} />
                Efetivar Orçamento
              </button>
